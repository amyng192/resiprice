#!/usr/bin/env python3
"""
fix_cortland_urls.py - Remediate the 6 Cortland properties in the failed bucket.

Two categories:
  1. Cortland at Buckhead Village + Druid Hills  → still Cortland-branded;
     DB URL points at retired individual .com domains (cortlandatbuckheadvillage.com,
     cortlanddruidhills.com). Redirect to the canonical cortland.com slug —
     the scraper already appends /available-apartments/ on its own.
  2. Glenridge, 5550, East Cobb, Holcomb Bridge → rebranded/sold; their
     cortland.com URLs 302 to perimetercircleliving, perimeter5550living,
     luminapts, broadleafholcomb respectively. All use SightMap so the
     scraper handles them; the first bulk run likely timed out before the
     iframe fully mounted.
"""
...
log = logging.getLogger("fix_cortland_urls")
...
REMEDIATIONS: dict[str, str] = {
    "Cortland at Buckhead Village": "https://cortland.com/apartments/cortland-at-buckhead-village/",
    "Cortland Druid Hills":         "https://cortland.com/apartments/cortland-druid-hills/",
    "Cortland Glenridge":           "https://perimetercircleliving.com/floorplans/",
    "Cortland 5550":                "https://perimeter5550living.com/floorplans/",
    "Cortland East Cobb":           "https://www.luminapts.com/marietta/lumin-east-cobb/conventional/",
    "Cortland Holcomb Bridge":      "https://www.broadleafholcomb.com/alpharetta/broadleaf-at-holcomb/conventional/",
}
...
def main() -> None:
    init_db()
    # Longer timeout for Cloudflare-gated luminapts/broadleaf pages
    ...
    for name, new_url in REMEDIATIONS.items():
        ...
        progress_record = {
            ...
            "notes": f"Cortland remediation: {pricing_info.get('notes', '')}; {scraper_note}",
...
if __name__ == "__main__":
    # Make remediation note match apply_remediations_to_csv's prefix allowlist
    from apply_remediations_to_csv import REMEDIATION_NOTE_PREFIXES
    assert any(p == "Cortland remediation" for p in REMEDIATION_NOTE_PREFIXES), \
        "Add 'Cortland remediation' to REMEDIATION_NOTE_PREFIXES in apply_remediations_to_csv.py"
    main()
